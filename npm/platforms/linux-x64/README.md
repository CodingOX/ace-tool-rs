# @ace-tool-rs/linux-x64

Platform-specific binary for [ace-tool-rs](https://github.com/CodingOX/ace-tool-rs) (Linux x64).

## Overview

This package contains the pre-built binary for Linux x64. It is automatically installed as an optional dependency when you run `npx ace-tool-rs` on Linux x64 systems.

**Do not install this package directly.** Instead, use:

```bash
npx ace-tool-rs --base-url <API_URL> --token <AUTH_TOKEN>
```

## Supported Platforms

- Linux x64/amd64

## License

This project is dual-licensed:

- **Non-Commercial / Personal Use**: GNU General Public License v3.0
- **Commercial / Workplace Use**: Commercial License Required

See the main [ace-tool-rs repository](https://github.com/CodingOX/ace-tool-rs) for full license details.

## Author

[missdeer](https://github.com/missdeer)
